...
min_mem = 120000  # min members required that have added
min_ep_duration = 18  # min episode duration in minutes
req_genres_all = ["comedy"]  # list of required genres all must be present
req_genres_either = ["comedy", "romance"]  # list of required genres one must be present
no_allow_genre = ["boys love", "ecchi", "hentai", "sports", "Erotica"]  # genres to remove
excluded_strings = ["To LOVE-Ru", "Date A Live", "Shinmai Maou no Testament",
                    "Baka to Test to Shoukanjuu", "High Score Girl", "Fate/"]  # strings that can't be in name
...
for line in lines:
    ...
    if allow:
        ...
        results.append(f"{link} {name} -- {duration}Mpe x {episodes} ({runtime}Hrs) Score:{score} Rank:{rank} "
                       f"Popularity:{pop} Members:{mem} Genres:{genres}".replace("\n", ""))
        count += 1

print(f"There are {count}/{len(lines)} anime's that meet your requirements")
while True:
    input(results[random.randint(0, len(results)-1)])
